...
export class OpenFileManagerOld {
    ...
    private async getSelectedFilePhysicPath(): Promise<string> {
        ...
        if (!selection.endsWith(".csproj")) {
            ...
            if (!fs.existsSync(p)) {
                vscode.window.activeTerminal?.sendText("echo p + ' not fond!")
                return '';
            }
        ...
        if (filter.length == 1) {
            if (isCoreProject) {
                ...
                if (!fs.existsSync(filter[0])) {
                    vscode.window.activeTerminal?.sendText("echo Have not been produced file.")
                    return '';
                }
            }
        ...
        if (tryFind.length > 0) {
            vscode.window.activeTerminal?.sendText("echo file not fond")
        } else {
            vscode.window.activeTerminal?.sendText("echo file not fond")
        }
                ...
                fs.stat(file, function (err, stat) {
                    if (stat && stat.isDirectory()) {
                        ...
                        if (file.endsWith(".csproj")) {
                            ...
                            var name = that.readAssemblyNameFromFile(file);
    ...
    async readAssemblyNameFromFile(fileName: string): Promise<string> {
        const fileStream = fs.createReadStream(fileName);

        const rl: string[] = readline.createInterface({
            input: fileStream,
            crlfDelay: Infinity
        });
        ...
        for await (let line of rl) {
            if (line.match("<AssemblyName>**")) {
                assemblyName = line.replace("<AssemblyName>", "");
                assemblyName = line.replace("</AssemblyName>", "");
                assemblyName = line.trim();
                break;
            }
        }
        return assemblyName;
